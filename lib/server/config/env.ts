...
export const env = {
  ...
  openAiBulkModel: process.env.OPENAI_BULK_MODEL ?? "gpt-5.6-luna",
  openAiStandardModel: process.env.OPENAI_STANDARD_MODEL ?? "gpt-5.4-mini",
  openAiComplexModel: process.env.OPENAI_COMPLEX_MODEL ?? "gpt-5.6-terra",
  ...
  vapidPublicKey: process.env.VAPID_PUBLIC_KEY ?? dotenvResult.parsed?.VAPID_PUBLIC_KEY ?? "",
  vapidPrivateKey: process.env.VAPID_PRIVATE_KEY ?? dotenvResult.parsed?.VAPID_PRIVATE_KEY ?? "",
  vapidSubject: process.env.VAPID_SUBJECT ?? dotenvResult.parsed?.VAPID_SUBJECT ?? "mailto:notifications@agodly.com",
